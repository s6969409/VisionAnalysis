...
namespace VisionAnalysis
{
    ...
    public class TepAlgorithmB : BaseToolEditParas
    {
        public TepAlgorithmB(ObservableRangeCollection<Nd> nodes) : base(nodes)
        {
            #region para value default...
            Inputs["InputImage"] = new PInput() { value = new Mat() };
            Inputs["contour"] = new PInput();
        ...
        public override Action actionProcess => () =>
        {
            ...
            Point[][] contours = Inputs["contours"].value as Point[][];
            double range = Convert.ToDouble(Inputs["range"].value);
            int index = Convert.ToInt32(Inputs["index"].value);
            #endregion
        ...
        #endregion
        private Mat pts2Mat(Point[] contour)
        {
            Rect rect = Cv2.BoundingRect(contour);
            Mat output = new Mat(rect.Size, MatType.CV_8UC1, Scalar.Black);
            int ofsX = contour.Select(c => c.X).Min();
            int ofsY = contour.Select(c => c.Y).Min();

            Parallel.For(0, contour.Length, i =>
            {
                output.Set(contour[i].Y - ofsY, contour[i].X - ofsX, 255);
            });
